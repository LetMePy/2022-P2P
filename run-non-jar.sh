...
compile()
{
    javac -d bin/ src/first-client/*.java src/client/*.java src/bench/*.java src/util/*.java
}
...
run_server()
{
    ...
    if [ $# -eq 1 ]; then
        java -classpath bin/ first-client.CentralIndexingServer $PORT
    else
        java -classpath bin/ first-client.CentralIndexingServer
    fi
}
